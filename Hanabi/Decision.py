...
def Decision_for_action(current_state:"State",current_agent:"Agent", lifes_left:int):
    ...
    best_play_index= 1
    best_hint_index, hinted_agent = (1, Player_2)


    current_probability_tensor = probability_matrix(current_agent,current_state.Discard_pile,current_state.Board_pile)
    hint_probability_tensor = probability_matrix(hinted_agent,current_state.Discard_pile,current_state.Board_pile)
    ...
    for i in range(5):
        for j in range(5):
            expected_outcome[1] += hint_probability_tensor[i,j,best_hint_index]*playable(hinted_agent.card_in_hand[best_hint_index], current_state.Play_pile)
